...
function app() {
	return {
		...
		titles: [], // id, title, date, starred, model(optional:last used model)
		title_diff_days: [],
		...
		get_communication_content(cid){
			...
			.then(response => {
				...
				if (msgs.length > 0 && msgs[msgs.length-1].role === "querying") {
					msgs.pop();
					this.messages = msgs;
					setTimeout(() => {this.get_streaming_content(cid,false);}, 50);
				} else {
					this.messages = msgs;
				}
		...
		UserEditMessageConfirm(event,src) {
			...
			if (src == 'enter') {
				...
				element = event.srcElement.parentElement.parentElement;
			}
			const msgind = element.childNodes[1].textContent;
			const msg = element.childNodes[3].value;
			if (msg.trim() == "") {
				return ;
			}
			let upload_messages = [];
			...
			.then(response => {
				...
				if (status == "error") {
					...
					setTimeout(() => {
						this.get_communication_content(response.data.cid);
					}, 0);
				}
			})
			.catch(error => {
